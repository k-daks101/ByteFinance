...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchAiBrief = async () => {
      try {
        ...
        if (isMounted) {
          setAiError(err?.message || "Unable to load AI daily brief.");
        }
      } finally {
        if (isMounted) {
          setAiLoading(false);
  ...
  const refreshAiBrief = async () => {
    try {
      ...
      setAiError(err?.message || "Unable to refresh AI daily brief.");
    } finally {
      setAiLoading(false);
    }
  };
      ...
      <section className="bf-card p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Generated by your backend OpenAI endpoint</p>
          </div>
          <button
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          <div className="rounded-lg border border-slate-100 p-4">
            ...
            <p className="mt-1 text-sm font-semibold text-slate-900">
              {aiLoading ? "Loading..." : aiBrief?.insight_date || "-"}
            </p>
          </div>
          <div className="rounded-lg border border-slate-100 p-4">
          ...
          <div className="rounded-lg border border-slate-100 p-4 sm:col-span-1">
            ...
            <p className="mt-1 text-sm font-semibold text-slate-900">
              {aiError ? "Unavailable" : aiLoading ? "Fetching" : "Live"}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-lg border border-slate-100 p-4">
          ...
          <p className="mt-2 text-sm text-slate-700 leading-6">
            {aiError
              ? "Could not load AI brief. Check backend API and CORS settings."
              : aiLoading
                ? "Loading AI insight..."
